...
func UpdateMetrics(config ScrapeConf, registry *prometheus.Registry, now time.Time, metricsPerQuery int, fips, debug, floatingTimeWindow, labelsSnakeCase bool) time.Time {
	tagsData, cloudwatchData, endtime := scrapeAwsData(config, now, metricsPerQuery, fips, debug, floatingTimeWindow)
